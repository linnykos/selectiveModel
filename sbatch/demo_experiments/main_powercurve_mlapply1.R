...
print("MLAPPLY 1 VERSION")

start <- proc.time()

res <- simulation::simulation_generator(rule = rule, criterion = criterion,
                                           paramMat = paramMat, trials = paramMat[,"trials"],
                                           cores = 1, as_list = F, verbose = T)

end <- proc.time()
print(end-start)


save.image(paste0("/home/kevinl1/selectivemodel/sbatch/results/TMP1_main_powercurve", paste0(args, collapse = "-"), ".RData"))
