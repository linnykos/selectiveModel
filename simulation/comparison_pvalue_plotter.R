...
#### DUMMY ####
load("../simulation/comparison_pvalue_saturated.RData")
zz <- res
...
for(i in 1:length(res)){
  ...
  plot(sort(tmp[1,]), seq(0, 1, length.out = length(tmp[1,])), xlim = c(0,1),
  ...
  legend("bottomright", c("Saturated", "Selected known sigma"), cex=0.8,
...
par(mfrow = c(1, length(res)))
for(i in 1:length(res)){
  plot(res[[i]][1,], res[[i]][2,], pch = 16, xlab = "Saturated", ylab = "Selected",
